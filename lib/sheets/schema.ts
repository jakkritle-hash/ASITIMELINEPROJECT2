...
export const TAB_HEADERS = {
  ...
  Projects: ['id', 'name', 'teamId', 'memberIds', 'ownerUserId', 'startDate', 'dueDate', 'status', 'description', 'kanbanColumns', 'createdAt', 'updatedAt'],
...
export function parseProject(r: Record<string, string>): Project {
  return {
    ...
    kanbanColumns: csvToArr(r.kanbanColumns), createdAt: r.createdAt, updatedAt: r.updatedAt,
  }
}
...
export function serializeProject(p: Project): Record<string, string> {
  return { ...p, memberIds: arrToCsv(p.memberIds), kanbanColumns: arrToCsv(p.kanbanColumns) }
}
